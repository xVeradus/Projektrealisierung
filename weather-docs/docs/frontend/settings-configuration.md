...
Die Komponente ruft bei relevanten Änderungen oder durch einen Klick auf "Suchen" die Methode `searchStations()` des `WeatherApiService` auf. Nach Erhalt der Antwort vom Backend aktualisiert sie den globalen State über `this.ui.setStations(stations)`, wodurch sich automatisch alle anderen Komponenten (wie die Karte) aktualisieren.
