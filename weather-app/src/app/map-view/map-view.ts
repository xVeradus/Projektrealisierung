 ...
 * Responsible for displaying weather stations, visualizing the search radius,
 * and handling drag-and-drop interactions for setting the search center.
 */
@Component({
...
export class MapViewComponent implements AfterViewInit, OnDestroy {
  ...
  /**
   * Calculates a dynamic search radius in kilometers based on the current
   * map zoom level and viewport bounds.
   */
  private getRadiusFromZoom(): number {
    ...
    return Math.max(1, Math.min(100, radiusInKm));
  }

  /**
   * Renders the center marker and search radius circle on the map.
   */
  ...
  /**
   * Places markers for the loaded weather stations and adjusts the map bounds
   * to fit all visible markers.
   */
  ...
  /**
   * Initiates the drag-and-drop operation for setting the search center.
   */
  ...
  /**
   * Handles the drag-over event to display a preview circle on the map.
   */
  ...
  /**
   * Handles the drop event, updating the UI state with the new center coordinates.
   */
