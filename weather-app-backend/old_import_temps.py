...
def download_from_ncei(station_id: str, dest: Path) -> None:
    ...
    Args:
        ...
        dest: Local path to save the .dly file.
    """
...
def download_from_s3(station_id: str, dest: Path) -> None:
    ...
    Args:
        station_id: NOAA station identifier.
        dest: Local path to save the .csv.gz file.
    """
    dest.parent.mkdir(parents=True, exist_ok=True)
    if dest.exists() and dest.stat().st_size > 0:
        return
...
def save_station_periods_to_db(conn: sqlite3.Connection, rows: List[Tuple]) -> None:
    ...
    Args:
        ...
        rows: Tuples formatted for insertion into station_temp_period.
    """
    conn.executemany(
        """
...
def import_station_periods(
    ...
    Args:
        ...
        ignore_qflag: If True, bad quality data is ignored.
        start_year: Optional bounding start year.
        end_year: Optional bounding end year.
    """
...
def ensure_station_periods_range(
    ...
    Returns:
        Dictionary indicating caching status and years fetched.
...
def get_station_periods(
    ...
    Args:
        station_id: NOAA station identifier.
        conn: Active SQLite database connection.
        start_year: Optional start year for query.
        end_year: Optional end year for query.

    Returns:
        A list of dictionaries containing period data.
    """
    create_schema(conn)
